...
| `RECACHED_WORKER_THREADS` | _(one per core)_ | Threads that execute commands. Unset means one per available core — inside a container that is the cgroup's CPU allowance, not the host's core count. Set it to share a machine with other processes, or to `1` to make command execution single-threaded the way Redis and Valkey run, which is how the [scaling benchmark](/guide/benchmarks#thread-scaling) isolates parallelism from hardware. A value that is not an integer in `1`–`1024` makes the server **refuse to start** rather than silently fall back. The count the server actually built is logged at startup. |
...
Available metrics include key count, command and error counters, active connections, replication lag, memory estimates, evictions, and WebSocket client count. Command latency histograms are not implemented.
